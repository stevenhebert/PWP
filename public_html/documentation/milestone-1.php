<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="UTF-8">
		<title>stevens personal website project</title>

	</head>

	<body>
		<h1>Purpose, Audience, Goal</h1>

		<ol>
			<li>purpose: I would like to personal website to showcase my web development skills and professional interests.</li>
			<li>audience: This webpage is designed for my professional peer group, both to collaborate with other developers and
				as a presence for my past works.</li>
			<li>goal(s): I maintain this page with the intention of building a clientele base</li>

		</ol>

		<h1>Persona</h1>

		<ul>
			<li>name: Gene</li>
			<li>description: Gene is a chief technology officer for a tech startup looking to hire budding junior
				developers</li>
			<li>goals: As a CTO, Gene is familiar with all aspects of a companies technological support.</li>
			<li>frustrations: His previous experience with both front end and back end has lead him to his current
				position. Often frustrated by then lack of coordination between these divisions, he's looking to hire developers
				that also understand this barrier. He feels this is especially important for startups, where synergy is vital
				to their survival.</li>
			<li>additional information:</li>

		</ul>

	</body>
</html>